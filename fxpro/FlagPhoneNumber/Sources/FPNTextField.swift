...
open class FPNTextField: UITextField {
		...
		if let phoneCode = selectedCountry?.phoneCode, let number = text {
			...
			if let validPhoneNumber = getValidNumber(phoneNumber: cleanedPhoneNumber) {
				...
				if let inputString = formatter?.inputString(cleanedPhoneNumber) {
                    let tempText = remove(dialCode: phoneCode, in: inputString)
                    text = tempText
                    if let formateText = formateOutputPhone(text: tempText) {
                        text = formateText
                    }
				}
				...
				if let dialCode = selectedCountry?.phoneCode {
					if let inputString = formatter?.inputString(cleanedPhoneNumber) {
						let tempText = remove(dialCode: dialCode, in: inputString)
                        text = tempText
                        if let formateText = formateOutputPhone(text: tempText) {
                            text = formateText
                        }
    ...
    private func formateOutputPhone(text: String) -> String? {
        var tempText = text
        guard let rangeFirst = tempText.range(of: " ") else { return nil }
        tempText.replaceSubrange(rangeFirst, with: "(")
        guard let rangeSecond = tempText.range(of: " ") else { return nil }
        tempText.replaceSubrange(rangeSecond, with: ")")
        return " \(tempText)"
    }
